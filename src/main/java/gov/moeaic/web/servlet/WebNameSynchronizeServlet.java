...
public class WebNameSynchronizeServlet extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException{
		...
		try{
			PreparedStatement stmt = sqltools.prepare(
				"SELECT id, photoFile, file_content FROM TB_WebName WHERE id = ? "
			);
			stmt.setInt(1, id);
			ResultSet rs = stmt.executeQuery();

			if(rs.next()){
				File file = new File(request.getServletContext().getRealPath(""), dTools.trim(rs.getString("photoFile")));
