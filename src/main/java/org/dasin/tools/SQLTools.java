...
public class SQLTools{
	...
	public SQLTools(int connectionType, String connectionString){
	...
	public static String toDBString(String inString){
		return dTools.dbstring(inString);
	}
	...
	public static ArrayList<String> getColumns(ResultSet rs){
		...
		try{
			...
			for(int i = 1;i <= rsmd.getColumnCount();i++){
				result.add(
					dTools.isEmpty(rsmd.getColumnLabel(i)) ? rsmd.getColumnName(i) : rsmd.getColumnLabel(i)
				);
			}
		}catch(Exception e){
			e.printStackTrace();
	...
	public static HashMap<String, String> getHashMap(ArrayList<String> columns, ResultSet rs){
		...
		try{
			for(int i = 0;i < columns.size();i++){
				...
				result.put(column, dTools.trim(rs.getString(column)));
			}
		}catch(Exception e){
			e.printStackTrace();
		}
